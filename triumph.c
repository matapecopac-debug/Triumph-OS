...
/*
 * triumph.c — Triumph OS Shell  (PID 1 capable, fully static)
 * A bash-like shell written in C.
 */
/* editor is included after its own types are needed — included near bottom */
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/* ── builtins ─────────────────────────────────────────────────────────────── */
static int run_line(char *line); /* forward */
...
static int b_fetch(Cmd *c){(void)c;
    ...
    const char *logo[]={
        " "CYN BLD"  ▀█████████▄  "RST,
        " "CYN BLD"     ████      "RST"  "WHT BLD"Triumph OS"RST,
        " "CYN BLD"     ████      "RST,
        " "CYN BLD"     ████      "RST"  "GRY"─────────────"RST,
        " "CYN BLD"     ████      "RST,
        " "CYN BLD"     ████      "RST,
...
static int b_help(Cmd *c){(void)c;
    ...
    const char *h[][2]={
        ...
        {"clear","clear screen"},{"nano/edit","text editor"},{"snake","snake game"},{"poweroff","power off the machine"},{"fetch","system info panel"},
...
/* ── editor (included here so it sees all types) ──────────────────────────── */
#include "editor.c"
#include "snake.c"
...
static BE btab[]={
    ...
    {"echo",b_echo},{"edit",b_edit},{"nano",b_edit},{"vi",b_edit},{"snake",b_snake},{"poweroff",b_poweroff},{"shutdown",b_poweroff},{"reboot",b_reboot},
...
static int exec_single(Cmd *cmd,int in_fd,int out_fd){
    ...
    /* if pure builtin with no redirection and in same fds, run inline */
...
static int run_line(char *line){
    /* strip comments */
    char *cm=strchr(line,'#');
    /* but don't strip # inside quotes — simple: only strip if # at start of word */
...
int main(int argc,char *argv[]){
    /* if given a script file, source it */
    if(argc>1&&argv[1][0]!='-'){
        Cmd dummy={.argc=2,.argv={argv[0],argv[1]}};
        return b_source(&dummy);}
    signals_init();
    setup_defaults();
    /* mount essential pseudo-filesystems if we're PID 1 */
    if(getpid()==1){
    ...
    while(running){
        ...
        hist_add(line);
        run_line(line);}
    if(getpid()==1){/* PID 1 never exits cleanly — reboot */
